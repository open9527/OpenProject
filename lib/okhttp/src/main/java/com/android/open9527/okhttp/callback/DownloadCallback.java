...
import com.android.open9527.okhttp.exception.MD5Exception;
import com.android.open9527.okhttp.exception.NullBodyException;
...
public final class DownloadCallback extends BaseCallback {
    ...
    protected void onResponse(Response response) throws Exception {
        ...
        if (parentFile != null) {
            HttpUtils.createFolder(parentFile);
        }
        ...
        if (!TextUtils.isEmpty(mMd5) && mFile.isFile() &&
                mMd5.equalsIgnoreCase(HttpUtils.getFileMd5(mFile))) {
            HttpUtils.post(() -> {
                if (mListener == null || !HttpLifecycleManager.isLifecycleActive(getLifecycleOwner())) {
                    return;
                }
        ...
        while ((readLength = inputStream.read(bytes)) != -1) {
            ...
            HttpUtils.post(() -> {
                ...
                int progress = HttpUtils.getProgressProgress(mTotalByte, mDownloadByte);
                // 只有下载进度发生改变的时候才回调此方法，避免引起不必要的 View 重绘
                if (progress != mDownloadProgress) {
                    mDownloadProgress = progress;
                    mListener.onProgress(mFile, mDownloadProgress);
                    HttpLog.print(mFile.getPath() + " 正在下载，总字节：" + mTotalByte + "，已下载：" + mDownloadByte +
                            "，进度：" + progress + " %");
                }
            });
        }
        HttpUtils.closeStream(inputStream);
        HttpUtils.closeStream(outputStream);

        String md5 = HttpUtils.getFileMd5(mFile);
